...
# ── Safe Imports to prevent fatal build crashes ──────────────────────────────
try:
...
# ═══════════════════════════════════════════════════════════════════════════════
#  SURVEY SESSION (RIDE & PIN) - NATIVE GPS INTEGRATION
# ═══════════════════════════════════════════════════════════════════════════════
with tab_survey:
    ...
    if not HAS_GEO:
        st.error("🛑 The Native GPS Tracker is missing. Please add `streamlit-geolocation` to your requirements.txt file to enable the map tools.")
    ...
    if not st.session_state["survey_active"]:
        ...
        st.markdown('<div class="sec-hdr">📍 STEP 1: Lock GPS Position</div>', unsafe_allow_html=True)
        
        # Ensure coordinates persist across form submits on mobile
        if "temp_lat" not in st.session_state: st.session_state["temp_lat"] = ""
        if "temp_lng" not in st.session_state: st.session_state["temp_lng"] = ""

        if HAS_GEO:
            # Native Streamlit GPS Button
            loc = streamlit_geolocation()
            
            if loc and loc.get('latitude') is not None:
                st.session_state["temp_lat"] = str(loc['latitude'])
                st.session_state["temp_lng"] = str(loc['longitude'])
            
            if st.session_state["temp_lat"]:
                st.success(f"🎯 GPS Locked: {st.session_state['temp_lat']}, {st.session_state['temp_lng']}")
            else:
                st.warning("⚠️ Tap the 'Get Location' button above to capture your coordinates.")
        ...
        if commit_pin:
            if not st.session_state["temp_lat"] or not st.session_state["temp_lng"]:
                st.error("❌ Location missing. Tap 'Get Location' first.")
                ...
                # Clear GPS Cache for the next pin
                st.session_state["temp_lat"] = ""
                st.session_state["temp_lng"] = ""
    ...
    if not active_techs or not active_locs:
        ...
        if f_sub:
            if q1 == 0 and q3 == 0:
                ...
                if is_dup: st.error(f"❌ Entry for **{tech}** on **{entry_date}** already exists. Edit it below.")
                else:
    ...
    if log_data.empty:
        ...
        if target_label != "-- Select --":
            ...
            with st.form("edit_log_form"):
                ...
                with ec1: e_q1 = st.number_input("1 PH Qty", min_value=0, step=1, value=curr_q1)
                with ec2: e_q3 = st.number_input("3 PH Qty", min_value=0, step=1, value=curr_q3)
                btn_update, btn_delete = st.columns(2)
                with btn_update: do_update = st.form_submit_button("✏️ Update", type="primary")
                with btn_delete: do_delete = st.form_submit_button("🗑️ Delete")

            if do_update:
                if e_q1 == 0 and e_q3 == 0: st.error("❌ Both quantities cannot be 0.")
                else:
    ...
    if df_inv_t.empty:
        ...
        if inv_target != "-- Select --":
            ...
            with st.form("edit_inv_form"):
                ...
                with ei1: e_qty = st.number_input("Quantity", min_value=1, step=1, value=safe_int(inv_row.get("qty", 1), 1))
                with ei2: e_mrn = st.text_input("MRN No.", value=str(inv_row.get("mrn", "")))
                with ei3:
                ...
                with ib1: inv_do_update = st.form_submit_button("✏️ Update", type="primary")
                with ib2: inv_do_delete = st.form_submit_button("🗑️ Delete")

            if inv_do_update:
                if not e_mrn.strip(): st.error("❌ MRN No. cannot be empty.")
                else:
            ...
            if inv_do_delete: st.session_state["pending_inv_del"] = inv_idx
...
with tab_admin:
    ...
    def process_editor_save(edited_df, worksheet_name, mandatory_cols):
        for m_col in mandatory_cols:
            if m_col not in edited_df.columns: return False, f"❌ Missing column '{m_col}'. Add a row using the '+' button."
        ...
        if df_clean.empty: return False, "❌ Cannot save: Table is completely empty. Leave at least one valid row."
        for m_col in mandatory_cols:
            if not df_clean[df_clean[m_col] == ""].empty: return False, f"❌ Validation Error: '{m_col}' cannot be blank."
    ...
    with subtab_tech:
        ...
        if df_t.empty: df_t = pd.DataFrame(columns=expected_cols_t)
        else:
    ...
    with subtab_loc:
        ...
        if df_l.empty: df_l = pd.DataFrame(columns=expected_cols_l)
        else:
            if "location_name" not in df_l.columns: df_l["location_name"] = ""
